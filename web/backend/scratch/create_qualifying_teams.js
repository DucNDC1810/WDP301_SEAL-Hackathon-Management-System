...
async function run() {
  ...
  // Set Round 2 to active and future deadline
  round2.is_active = true;
  round2.submission_deadline = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days from now

  await contest.save();
  console.log("Updated Contest rounds statuses and deadlines in Contest document.");

  // Also update standalone Round documents
  await Round.updateOne({ _id: round1._id }, { $set: { is_active: false } });
  await Round.updateOne({ _id: round2._id }, { $set: { is_active: true, submission_deadline: round2.submission_deadline } });
  console.log("Updated standalone Round documents.");
