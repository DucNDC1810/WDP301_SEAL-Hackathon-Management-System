    ...
    fetch(`${API_URL}/api/teams/verify?token=${token}`)
      ...
      .then((data) => {
        if (data.success) {
          ...
          setMsg(data.message || 'Email đã được xác thực. Chào mừng bạn đến với đội thi!');
        } else {
          setStatus('error');
          setMsg(data.message || 'Xác thực thất bại. Vui lòng thử lại.');
        }
      })
      .catch(() => {
        setStatus('error');
      ...
      <div className="w-full max-w-[440px] bg-[rgba(17,24,39,0.8)] backdrop-blur-xl border border-[rgba(0,240,255,0.15)] rounded-2xl p-8 text-center">
        ...
        {status === 'loading' && (
          <div className="flex flex-col items-center gap-4">
            <Spin size="large" />
            <p className="text-[#f1f5f9] text-lg font-semibold">Đang xác thực...</p>
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-[#f1f5f9] text-xl font-bold">Xác thực thành công!</p>
            {teamName && (
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-[#f1f5f9] text-xl font-bold">Xác thực thất bại</p>
            <p className="text-[#94a3b8] text-sm">{msg}</p>
            <div className="flex gap-3 mt-2">
              <Button type="primary" onClick={() => navigate('/dashboard')}>Về Dashboard</Button>
              <Button onClick={() => navigate('/')}>Trang chủ</Button>
            </div>
          </div>
        )}
